...
- **Accordion** — pin existing grid tiles, or type up to twenty label+command rows. In **Edit buttons**: tap the parent, dismiss the menu, then **long-press** another tile to pin it (toast: Pinned to MORE). Tap a pinned tile and choose **Unpin from "MORE"**. A tile belongs to one parent only. Pinned tiles hide in play until the parent opens, then they appear where you placed them. **Accord.** still has direction, Open with (tap/hold/swipe), typed rows for wizard packs, and **Columns / Rows** (type `2` for two columns). The gesture that opens the accordion cannot also send its own command. Super / floating buttons cannot have an accordion. Editor badges: **T** tap, **H** hold, **S** swipe. Options can draw gesture hints (uncheck to hide **U/D/L/R**, diagonal arrows, Hold, and accordion badges).
