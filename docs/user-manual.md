...
cell — that drops a blank tile. Tap the tile to select it, tap again for
Move / Edit / Copy / Delete, choose **Edit** to edit the button.
...
The ⋮ menu on the server list shows whether **Manage Storage Access**,
...
**It is kept per world, and it travels with a full launcher backup.** Each MUD
has its own bag, because `kill` means something different on each of them.
Session vocabulary (names the game just printed) is forgotten when you
connect, so one world's mob names are never offered in another. Pairings
your own commands taught live in a `wordpairs-*.xml` sidecar next to the
world's settings in app storage. **Export Settings** writes the world XML
only; **Backup All Settings** on the launcher zips every `.xml` there,
including that sidecar.

It is small: a few kilobytes for ordinary play. Pairings cap at 100 verbs
and 40 targets each.
...
- **Also on this line** → optional second gate after Pattern and Match
  ...
  that channel's spam and leaves a minimap `--` alone. Two colours on
  two spans: make another trigger for the other Pattern + Match style,
  then **Conditions** → *Only if trigger matches this line* and pick it
  (look-ahead; it does not run that trigger's actions). Match style
  still applies only to this trigger's Pattern span.  
...
- **Group** → optional label (e.g. `combat`); blank = ungrouped. Use the
  ...
  **More** is Enable/Disable all for the current filter, and on Triggers
  a sort-by-sequence toggle. Group filter is the spinner under search, not
  that menu.
- **Conditions** → extra gate after the pattern matches — not a substitute
  ...
  `plugin:name` ok), **Only if trigger matches this line** (that trigger's
  Pattern + Match style + Also on this same line; not its actions or
  conditions; off/missing/self/timer = closed), Alias enabled/disabled, Alias replacement equals,
...
similar actions. A Notification shade title is the world name, then the title
you typed, so several sessions can be told apart. `$` on its own is end of line, not a dollar sign: to match
...
**Sample — gag a channel line, leave a minimap `--`:** trigger `tag` with
Pattern `[chan]:` and Match style on the tag colour. Trigger `dash` with
Pattern `--` and Match style on the dash colour. Trigger `spam` with the
same `--` pattern and dash style, Conditions → *Only if trigger matches
this line* → pick `tag`, action Gag. A minimap `--` has no tag, so `tag`
would not match that line and the gag stays closed. `tag` and `dash` need
no actions; they are the conditions `spam` looks at.
...
Enable/Disable all (Triggers/Aliases) stays under **More**. Triggers
...
replacement equals, Variable equals/exists/below/above. *Only if trigger
matches this line* stays closed on a timer (no game line). Set vars with the **Set
...
- **The sounds that ship with BlowTorch.** These can never go missing and
...
**Auto-Send Path?** is on (off by default). `.map go <room>` always walks.
...
instead of a bare “Loading…”. The box filters file names
...
are written. Files are `{world}_{yyyy-MM-dd}.txt` (one file per world per
local day; reconnects append). Older `{world}_{yyyy-MM-dd_HH-mm-ss}.txt`
files still list. Tap a file to
...
### Commands
    ...
    `.map goto <query>`                                                     Path; send only if **Auto-Send Path?** is on (default off)
...
recording defaults, follow, **Auto-Send Path?**, **Use GMCP Room Sync?**,
**Configure Room Sync…** (room number / absolute coords default off / create exits),
...
**GMCP Room:** with **Use GMCP?** and **Use GMCP Room Sync?** on (and **Room** in Manage
modules…), `Room.Info` builds the map as you walk:
- **num/id/vnum** → stable tile identity (IRE games and others)
- **coords** / **coord** `{x,y,z}` → place on the grid only if **GMCP: Use
  absolute coordinates?** is on (off by default). `z` still picks a floor
  when grow is on
...
stop working; they stay visible in scrollback. Bare `<EXPIRE>` clears
SEND/A links that had an expire group; links with no group stay tappable.
...
- `SEND` (tap sends a command), menus (`|` commands), `PROMPT` (fills the
  ...
  scrollback). Bare `<EXPIRE>` clears SEND/A that had an expire group.
...
.frame reopen <id>    — put back one you closed (`.frame open` is the same)
.frame reopen         — what can come back
```
...
(when Log Session to File? is on), including while Use MCP? is off.
