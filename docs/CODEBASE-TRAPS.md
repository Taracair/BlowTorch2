...
| Assumed colour triggers, bleed, or bold made fling janky | Probe `drawTextOnGrid` first. 5 Sep 2026, bold off (`weight=0`): colour units 1–2ms cache hits, bleed already answered (2ms / 9 lines); `gridMs` 31–34 of a 40–49ms frame was clips then per-unit `drawText`. 6 Sep 2026 settled dense-colour fling: `addMs=0`, `hw=1`, `weight=0`, `clips` 0–1, `colorMs=3`, `gridMs` 13–20 of worst 32–38ms, ~900–1100 `drawText`. No-glyph walk the same day: worst 19–22ms (`gridMs=0`, `restMs` ~12, `pinScan` 5–6). Paper fill `<1ms`. Those clip/coalesce/grid fixes were real; they were not a 16ms fling. Do not add a fourth `drawText` pass. |
| Kept typesetting every glyph on every fling frame after a week of clip, coalesce, and grid work | That method lost. Idle typeset is the right engine for a still screen. Motion uses per-line tiles. A new clip, a per-glyph loop, or “one more coalesce” is repeating the method that already lost. Do not retry a fourth `drawText` pass. |
...
| Does bare-CR-to-newline double-space SMAUG? | It did, when the wire is LFCR. 12 Sep 2026 TCP dump of a SMAUG banner: 32 LFCR, 2 CRLF, 0 bare CR. LFCR is now one newline; bare CR between map rows is still a newline. Do not drop the CR→LF conversion. Tests in `CrToNewlineTest` (`lfcrIsOneNewline`). |
