...
| Does Word Wrap break ASCII maps? | It used to, at the spaces inside `[ ]-[ ]` tiles. Those lines (and box-drawing / Block Element maps) hard-break at the column. 4 Sep 2026: Liberation Mono 28, cell=17, wrapCol=85, mini-map `breaks=0`. Flying mini-map rows in the buffer were not the same width because Processor dropped bare CR, so three 10-cell rows joined into one 30-character sky line. Walking `lmap` rows used LF and stayed 78 columns. Bare CR is now a newline; CRLF is still one. |
