...
| Does CSI eat ASCII map tiles? | It did, when a colour code had no `m` yet. Final bytes `@`-`~` include `[ ] | \\ ` o O @`. Those were consumed as commands, and a newline inside CSI glued the next row on. 4 Sep 2026: only SGR `m` applies colour; cursor/erase/MXP `z` are skipped; other finals and a newline abort and are reprocessed as text. |
