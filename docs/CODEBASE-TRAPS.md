...
| Does Word Wrap break ASCII maps? | It used to, at the spaces inside `[ ]-[ ]` tiles. Those lines (and box-drawing / Block Element maps) hard-break at the column. 4 Sep 2026 on a live world: Liberation Mono 28, cell=17, wrapCol=85, mini-map `breaks=0`. Flying mini-map rows in the buffer are not the same width: title rows are 5 tiles (10 cells) plus text; sky-only rows are 30 characters (fifteen `oO` tiles). Walking `lmap` rows are all 78 columns with spaces. SGR on the flying tiles is ANSI 46/37/1 and xterm 256, not italic. That session's column stretch is those extra tiles, not glyph advance. |
