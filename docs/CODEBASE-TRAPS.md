...
| Does Word Wrap break ASCII maps? | It used to, at the spaces inside `[ ]-[ ]` tiles. Those lines (and box-drawing / Block Element maps) hard-break at the column. 4 Sep 2026: Liberation Mono 28, cell=17, wrapCol=85, mini-map `breaks=0`. A live world's flying `map` rows were already 30 characters (15 two-char tiles) with LF, not three 10-cell rows glued by CR — converting bare CR to LF left those lengths unchanged (session log before vs after). LOOK mini-map is a 12-column gutter plus the room text. Walking `lmap` is 78 columns and lines up in the log. Jagged cyan is colored `oO` tiles; the rest of the row is spaces without that background. |
