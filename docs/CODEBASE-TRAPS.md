...
| Does Word Wrap break ASCII maps? | It used to, at the spaces inside `[ ]-[ ]` tiles. Those lines (and box-drawing / Block Element maps) hard-break at the column. 4 Sep 2026 on a live world: Liberation Mono 28, cell=17, every sampled ASCII glyph 17px, wrapCol=85, mini-map `breaks=0`. That session's column drift is not typeface advance or word-wrap. |
