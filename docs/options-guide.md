...
| **Sensors…** | — | The list screen, built like the Alias / Trigger / Timer chooser: readings grouped under *A hand over the screen*, *Movement*, *Light* and *Headphones, charger and screen*, each row one line of what it is or what already answers it, with a pencil (open the trigger editor) and a play (fire it here and now, dim until something answers). Readings this handset cannot provide fold away under *Not available on this phone*, still tappable, because a profile is shared with people whose phones do have them. The `?` in the button bar is what the screen is for; which chip provides a reading is `.sensor caps`, not the row |
