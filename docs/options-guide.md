...
| **Toolbar actions** | CSV for the left map buttons (`record,follow,level-,level+,find,undo,center,close`; optional `capture`). **Links**, **Paths/Pack**, **Levels**, **◎**, **Draw**, **Here**, **Edit**, **Save** are always appended. **L−**/**L+** nest down/up from Here (per-tile pocket floors), not a global stack |
...
tile-anchored levels (**Levels** browser, ▲/▼/◆ badges, **L−**/**L+** nests),
