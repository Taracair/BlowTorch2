...
function makeUI(editorValues,numediting)
  ...
  if(ui.advancedPage == nil) then
    ...
    help:setText("Name labels the button in the editor. Target Set is the button-set name used by switch-to commands. Tap a color swatch to change it; long-press a swatch to reset that color to the set default. Width, height, and position are in dp from the top-left of the button layer.")
