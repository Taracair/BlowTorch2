...
public class TimerEditorDialog extends Dialog implements DialogInterface.OnClickListener,TriggerResponderEditorDoneListener {
	...
	static final String TIMER_HELP_TEXT =
			...
			+ "Variable is below / above compare as numbers. Variables are session "
			+ "sticky notes (Set Variable / Lua SetVariable); Keep after restart is on "
			+ "the Set Variable action. Use ${name} in alias or action text.\n\n"
			...
			+ "    .timer info <name>       status (state, set for, elapsed, remaining, repeat)\n"
			+ "    .timer info <name> window  same text in the game window\n"
			+ "    .timer dump <name>       same as info … window; dump with no name lists all\n"
